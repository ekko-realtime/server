...
class AssociationsMgr {
  constructor({ setLoadInterval, io }) {
    const { logEvent } = require("../bin/logging")(io);
    this.logEvent = logEvent;
    console.log("setting logEvent ", logEvent);
    this.loadData();

    //pass in boolean for whether or not you want to repeatedly
    if (setLoadInterval) {
      this.syncDataInterval = setInterval(this.loadData.bind(this), 60000);
    }
  }
  ...
  logTest(event) {
    ...
    this.logEvent({ socket, eventName: event });
  }
